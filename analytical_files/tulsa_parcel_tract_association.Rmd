---
title: "tulsa_parcel_tract_association"
author: zachandfox
...
# Import raw CoT data #
cot_raw <- read_csv(file.path(tps_project_directory, term, "input/TulsaParcelsNeighborhood.csv"))
...
cot_tps_tract_association <- cot %>% 
  ...
  mutate(tract_number = paste0(STATEFP10, COUNTYFP10, TRACTCE10)) %>% 
  
  # Move geometries to front columns 
  select(-x1) %>% 
  select(fid:lat, STATEFP10:tract_number, everything()) %>%
  # select(fid:lat, STATEFP10:tract_number, everything()) %>% 
...
write_csv(cot_tps_tract_association, file.path(project_directory, term, "output/tulsa_parcels_with_tracts/tulsa_parcel_tract_association.csv"))
